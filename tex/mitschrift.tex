...
\newcommand{\FT}[1]{	% typset definition for Fourier transformation operator
  \mathcal{FT}\{ #1 \}
}

\newcommand{\princVal}{  % typeset cauchy principle value
  \mathcal{P}
}
...
\begin{figure}[t]
  \begin{minipage}[c]{.50\textwidth}
    \vspace*{\fill}
    \centering
    \includegraphics[width=0.9\linewidth]{../img/causal_suscept.pdf}
%   \captionsetup{width=6.9cm}
  ...
  \begin{minipage}[c]{.50\textwidth}
    \vspace*{\fill}
    \centering
    \includegraphics[width=0.9\linewidth]{../img/causal_sign_fct.pdf}
    \captionsetup{width=.95\linewidth}
  \end{minipage}
  \captionof{figure}{\textbf{Left:} An example of a response function obeying causality as stated by \refEq{eq:response_causality}. \textbf{Right:} Graphical illustration of the sign function.}
  \label{fig:QO_landau_level}
\end{figure}
...
%Since the Fourier transform conserves the symmetry, the Fourier transform of $\chi_\text{odd}(t)$ is also odd. Furthermore, 
...
%
%\begin{equation}
%  P ~~=~~ \frac{1} \int_0^T f(t) \cdot \dot{x}(t)\ dt ~~=~~ \frac{f_0^2}{T} |\chi(\omega)| \int \cos(\omega t) \omega_0 \cos(\omega t - \phi)\ dt ~~=~~ 
%\end{equation}
...
%It is also nice to see, that the very physical distinction between energy transmission and -dissipation is mapped to a purely mathematical property like real- and imaginary part of the systems characterizing quantity like the susceptibility.
...
% But this relation also holds if we assume the current density and the electric field to have a frequency dependence
...
%    \vspace*{\fill}
    \centering
    \includegraphics[width=0.75\linewidth]{../img/QO_landau_zylinder_2D_.png}
    \captionsetup{width=.95\linewidth}
  \end{minipage}
  \captionof{figure}{\textbf{left}: Occupied states in reciprocal space of a free electron gas in a magnetic field. The accessible energy states are inside the sphere bound to lay on cylinders with radius $\vc{k}_\perp = \sqrt{(n+1/2)2m \omega_c/\hbar}$. \textbf{right}: top sketch shows accessible states of electrons in a plane in reciprocal space for $B=0$. Bottom picture shows accessible states now arranged in circles due to $B \neq 0$.}
  \label{fig:QO_landau_level}
\end{figure}
...
%    \vspace*{\fill}
    ...
    \captionsetup{width=.95\linewidth}
  \end{minipage}
    \captionof{figure}{\textbf{a.} and \textbf{b.} show quantum oscillation measurements data on resistivity and magnetic torque. Oscillations of the measured quantities are clearly visible in the graphs inset. \textbf{c.} Oscillatory component of the data from \text{b} plotted over $1/B$. Figures are adapted from \cite{Vignolle2008}.}
...
\section{Free electron gas} % 10.10.17 - first part
...
%   \captionsetup{width=6.9cm}
  ...
  \begin{minipage}[c][6.00cm]{.5\textwidth}
    ...
    \captionsetup{width=.95\linewidth}
 %   \captionsetup{width=6.9cm}
    \captionof{figure}{Illustration of  inverse susceptibility $\chi$ of a Para- and Ferromagnet.}
  \end{minipage}
...
%   \captionsetup{width=6.9cm}
    \captionsetup{width=.95\linewidth}
    \captionof{figure}{Illustration of the magnetic field decrease inside the superconductor according to \myEq{eq:B_decrease}. The magnetic field is pictorially drawn as blue vectors. Adapted from \cite{Kittel2004}}
...
%   \captionsetup{width=6.9cm}
  ...
  \begin{minipage}[c][6.00cm]{.5\textwidth}
    ...
    \captionsetup{width=0.95\linewidth}
 %   \captionsetup{width=6.9cm}
    \captionof{figure}{Wave function in one dimension according to \myEq{eq:vortex_wave_function}. Comparing the blue and red curves shows that the wave function gets \textit{pushed down} for increasing coherence length ($\xi_2 > \xi_1$).}
    \label{fig:london_lambda}
  \end{minipage}
\end{figure}

We can now use our expressions for the penetration depth (\myEq{eq:penetration_depth_temp_dep}) and the coherence length (\myEq{eq:vortex_coherence_length}) to express the Ginzburg-Landau parameter $\kappa$ as
...
Each vortex has one flux quantum $\Phi_0$. The diameter of a vortex is given by the coherence length $\xi$. Looking at a cut through a superconductor with edge length $L$,  we can define the total surface of the material covered by vortices as

\begin{equation}
  2\pi \xi^2 N ~~=~~ A_\text{vortex}
\end{equation}

where $N$ is the number of vortices in the material. 
Looking for the situation where $A_\text{vortex} = L^2$ we can say, that the superconducting state is fully repressed by the vortex inherent magnetic field. 
...
Since one vortex has exactly one flux quantum, we can set up a relation between the penetration depth $\lambda$ and a lower critical field $H_{c_1}$. This lower critical field marks the point when vortices start to be \textit{produced} at the materials surface. 
